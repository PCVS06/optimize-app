---
name: optimize-wiki
description: Maintain Optimize company knowledge. Use when creating or editing articles, organizing overview pages, adding links, importing evidence, resolving contradictory content, restoring a previous version, or moving articles into and out of Trash. Also use before answering company questions from Wiki sources.
---
...
Use `optimize_wiki_history` and `optimize_wiki_revision` only for history/comparison/restoration. Historical pages are not current evidence. Restore by publishing the chosen content against the latest revision; this creates a new revision and retains intervening history. Parent changes must not create cycles. For a deletion request, read the article and use `optimize_wiki_archive` with `archived: true` and its current revision. This moves one article to recoverable Trash. Its subpages remain available at Wiki home; links become usable again when it is restored. For a restore request, find the article using `optimize_wiki_trash`, then pass its current Trash revision with `archived: false`. The same page ID and version history are retained. Trashed articles are excluded from current company answers. Only read their history when explicitly requested. Keep all deletion and restoration inside these tools; there is no permanent-delete operation. Mention duplicate or broken links when they materially affect the requested work.
