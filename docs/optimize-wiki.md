...
Two read-only host tools are registered in the existing agent tool catalog: `optimize_wiki_search` and `optimize_wiki_read`. Where the runtime adapter cannot use host tools, guidance identifies the current JSON page directory for its file-reading tools. Stock Pi retrieval and title citations were validated through this path. Inaccessible sources must be reported as unavailable.
